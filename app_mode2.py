...
def _restore(info):
    ...
    st.session_state.final_info_confirmed = False
    st.session_state.final_report_selection = {field: info[field] for field in ("year", "half", "start", "end")}
    st.session_state.final_confirmed_snapshot = False
...
def render_mode2():
    ...
    st.session_state.setdefault('final_info_confirmed', st.session_state.get('final_confirmed_snapshot', False))
    ...
    if result and info['warnings']:
        st.warning('날짜 자동 인식에 확인이 필요한 항목이 있습니다. 생성 조건의 인식 근거를 확인해 주세요.')
        confirmed = st.checkbox('보고연도·보고구분·기간을 확인했습니다', key='final_info_confirmed')
        st.session_state.final_confirmed_snapshot = confirmed
        valid = valid and confirmed
    ...
    with st.container(border=True, key='final_steps'):
        ...
        with c2:
            with st.container(border=True, key='final_generate_step'):
                _heading(2, '반기보고서 생성', '공사·용역·물품 집계와 수정값을<br>확정 양식에 반영합니다.', 'green')
                if st.button('반기보고서 생성하기', key='final_generate', width='stretch', disabled=not valid):
                    st.session_state.final_source_result = reports.build_final_halfyear_report_bytes(upload.getvalue())
                    st.session_state.final_generation_notice = True
                    st.rerun()
        ...
        st.markdown('<div class="final-workflow"><b>ⓘ 작업 순서 안내</b><span>❶ 자료 입력 → ❷ 보고 정보 확인 → ❸ 반기보고서 생성 → ❹ 결과 검토 → ❺ 다운로드</span><details><summary>사용방법 보기</summary>검토용 기초자료 1개를 업로드하고 자동 인식된 보고 정보를 확인하세요. 수정한 제목·기간·파일명은 즉시 보고서에 반영됩니다. 생성 버튼은 현재 조건으로 보고서를 다시 만듭니다.</details></div>', unsafe_allow_html=True)
    if st.session_state.pop('final_generation_notice', False):
        st.success('현재 보고 정보로 반기보고서를 생성했습니다.')
    if notes:
        st.warning(' · '.join(notes))
    st.markdown('<div class="final-sheets"><b>시트 구성 미리보기</b><span>▦ 공사</span><span>▦ 용역</span><span>▦ 물품</span><span>▦ 검토반영</span></div>', unsafe_allow_html=True)
    if result:
