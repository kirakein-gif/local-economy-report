...
st.markdown(
    f'''<div class="app-head"><div class="app-brand"><div class="app-logo">▦</div><div><div style="display:flex;align-items:baseline;gap:9px;flex-wrap:wrap"><div class="app-title">지역경제활성화 자동 집계</div><span style="font-size:.68rem;font-weight:600;color:#A1A1AA;white-space:nowrap">v{APP_VERSION} · 배포 {DEPLOY_DATE}</span></div><div class="app-sub">{mode_caption}</div></div></div><div class="live-chip">사용 {active_count}/{MAX_CONCURRENT} · 대기 {waiting_count}</div></div>''',
    unsafe_allow_html=True,
)
