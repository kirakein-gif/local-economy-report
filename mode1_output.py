...
from core_logic import categorize_region_code, extract_source_metadata, format_money, safe_value
...
def render_mode1_outputs(ctx):
    ...
    institution_name, school_level = extract_source_metadata(df, ctx['headers'])
    ...
    with col1:
        with st.container(border=True):
            ...
            try:
                ...
                st.download_button(
                    '분기보고서 다운로드', q, f'지역경제활성화_실적보고({target_region}기준).xlsx',
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', width='stretch', key='quarter_download',
                )
    ...
    with col2:
        with st.container(border=True):
            ...
            with h2:
                meta = ' · '.join(x for x in [institution_name, school_level] if x)
                desc = '확정 1-4 양식 · 원본 기관정보 자동 반영' + (f' · {meta}' if meta else '')
                st.markdown(f'<div class="card-title">반기 검토용 기초자료</div><div class="card-desc no-margin">{desc}</div>', unsafe_allow_html=True)
            try:
                b, n, _ = reports.build_review_workbook_bytes(
                    df, ctx['headers'], ctx['target_amount'], target_region,
                    institution_name, school_level,
                    ctx['report_year'], ctx['report_label'], ctx['period_start'], ctx['period_end'],
                )
                ...
                st.download_button(
                    ...
                    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', width='stretch', key='review_download',
                )
            except Exception as exc:
