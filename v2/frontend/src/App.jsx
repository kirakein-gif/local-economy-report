...
export default function App() {
  ...
  const foundByBiz = useMemo(() => {
    ...
    for (const item of addressResult?.results || []) {
      if (item.found && item.biz_no && item.address) {
        map[item.biz_no] = item.address;
      }
    }
    return map;
  }, [addressResult]);
  ...
  const enteredManualCount = useMemo(
    () => unresolvedCandidates.filter((item) => String(manualAddresses[item.lookup_key] || "").trim()).length,
    [unresolvedCandidates, manualAddresses]
  );
  ...
  function setManualValue(key, value) {
    setManualAddresses((current) => ({ ...current, [key]: value }));
    setReviewInfo(null);
  }
  ...
  async function saveManual(candidate) {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.detail || "공유 저장에 실패했습니다.");
  ...
  async function createReviewWorkbook() {
    ...
    try {
      const response = await fetch("/api/prepare/review", {
        method: "POST",
        body: form,
      });
      if (!response.ok) {
  ...
  const manualStoreName = config?.manual_address_backend === "firestore" ? "수동주소 공유 저장" : "수동주소 로컬 저장";
      ...
      <main className="main">
        ...
        {mode === "prepare" ? (
            ...
            {result && (
              <section className="results">
                ...
                <div className="result-foot">
                  기관: <b>{result.institution || "자동 확인 실패"}</b> · 자동 감지 지역: <b>{result.auto_region}</b> · API 조회 후보:
                ...
                <div className="address-action">
                  <div>
                    ...
                    <h3>공공 API · 저장주소 조회</h3>
                    <p>사용자 저장주소를 먼저 확인한 뒤 나라장터 → 학교장터(S2B) → 공정위 → 지역화폐 순으로 조회합니다.</p>
                  </div>
                  <button
                    className="primary"
                  ...
                  <>
                    <div className="address-result">
                      <div className="address-summary">
                        ...
                        <div>
                          <span>미확인</span>
                          <strong>{formatNumber(unresolvedCandidates.length)}개</strong>
                        </div>
                        <div>
                          <span>캐시 재사용</span>
                        ...
                        <div>
                          <span>저장주소 재사용</span>
                          <strong>{formatNumber(addressResult.manual_hit_count)}개</strong>
                        </div>
                      </div>
                      <div className="source-line">
                        저장주소 <b>{formatNumber(addressResult.source_counts?.["사용자 저장주소"])}</b>
                        <span>·</span>
                      ...
                      <div className="manual-panel">
                        <div className="manual-head">
                          <div>
                            ...
                            <p>입력한 주소는 이번 검토파일에 반영됩니다. 사업자번호가 있으면 공유 저장도 가능합니다.</p>
                          </div>
                            ...
                            return (
                              <div className="manual-row" key={candidate.lookup_key}>
                                ...
                                <input
                                  ...
                                  value={manualAddresses[candidate.lookup_key] || ""}
                                  placeholder="확인한 업체 주소를 입력하세요"
                                  onChange={(event) => setManualValue(candidate.lookup_key, event.target.value)}
                                />
                                <div className="manual-buttons">
                                  ...
                                  <button
                                    className="secondary"
                                    disabled={!candidate.biz_no || !String(manualAddresses[candidate.lookup_key] || "").trim() || status === "saving"}
                    ...
                    <div className="review-panel">
                      <div>
                        ...
                        <h3>검토용 기초자료 생성</h3>
                        <p>
                          API 확인 주소와 직접 입력 주소를 원본 데이터의 빈 주소에 반영한 뒤 기존 공식 1-4 서식으로 생성합니다.
                        </p>
                      </div>
                      <button className="primary" disabled={reviewBusy} onClick={createReviewWorkbook}>
                        {reviewBusy ? "Excel 생성 중..." : "검토용 Excel 다운로드"}
                      </button>
                    </div>
          ...
          <section className="empty-card">
            ...
            <h2>반기보고서 최종작성</h2>
            <p>다음 단계에서 검토 완료 파일을 받아 기존 4시트 반기보고서 생성 로직을 FastAPI로 연결합니다.</p>
          </section>
        )}
      </main>
    </div>
